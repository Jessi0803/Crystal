...
- 當顧客描述能量需求（如招桃花、提升人緣、改善健康、化解小人等），若知識庫中沒有完全符合的現貨商品，主動推薦客製化手鍊服務，說明可以依需求量身設計，並附上連結 https://goodaytarot.com/custom
...
- 若已有【可推薦的相關商品】清單，禁止寫「沒有推薦商品」「沒有現貨可推薦」「沒有符合的現貨」等否定整份現貨清單的句子；若顧客指定的主石或款式不在清單上，可簡短說明後，仍須依清單逐款介紹現貨並附上該清單的完整 https 連結
...
/**
 * 回覆若明確否定「無推薦／無現貨」卻未引用本次清單內連結或品名，不應再顯示商品卡，避免與文案矛盾。
 */
function replyDeclinesListedProductsWithoutCitation(
  reply: string,
  ...
  for (const p of related) {
    ...
    if (reply.includes(abs) || reply.includes(`/products/${p.id}`)) return false;
  }

  for (const p of related) {
    const n = p.name.trim();
    if (n.length >= 2 && reply.includes(n)) return false;
  }
  ...
  return negPatterns.some((re) => re.test(reply));
}
  ...
  chat: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (relatedProducts.length > 0) {
        ...
        ragContext +=
          `\n\n【可推薦的相關商品（以下 ${n} 款即聊天室將顯示的商品卡；回答中推薦的現貨必須與此清單完全一致）】\n` +
          relatedProducts
            .map(
              (p, i) =>
      ...
      if (!hasGoodMatch) {
        ragContext +=
          "\n\n【備援資訊】\n" +
          "若顧客的需求在現有商品中找不到完全符合的款式，可推薦「客製化水晶手鍊」服務。" +
          "老闆會根據顧客的功效需求（如招桃花、改善健康、化解小人、提升某種特定運勢等）量身設計專屬手鍊。" +
      ...
      let productsForClient = relatedProducts;
      if (
        productsForClient.length > 0 &&
        replyDeclinesListedProductsWithoutCitation(
          reply,
          productsForClient.map((p) => ({ id: p.id, name: p.name }))
        )
      ) {
        console.warn(
          "[chatbot] omitting relatedProducts: reply declined stock without citing listed items"
        );
        productsForClient = [];
      }
