...
export function LineFriendRewardCard() {
  ...
  return (
    <div className="mb-6 rounded-lg bg-white border border-[oklch(0.9_0.03_150)] px-5 py-4 flex flex-col sm:flex-row sm:items-center gap-3">
      <div className="flex-1">
        <p className="text-sm font-medium text-sf-ink" style={{ fontFamily: "'Noto Sans TC', sans-serif" }}>
          LINE 好友禮 · 折 {amount}
        </p>
        <p className="text-xs text-sf-muted font-body mt-0.5 leading-relaxed">
          {status.lineBound
      ...
      {status.lineBound ? (
        <div className="flex gap-2 shrink-0">
          <a
            ...
            className="text-xs font-body border border-sf-line-strong px-4 py-2 text-sf-text hover:bg-sf-selected"
          >
            加入好友
          </a>
          <button
            ...
            className="rounded-full text-xs font-body bg-[#06C755] text-white px-4 py-2 hover:opacity-90 disabled:opacity-60"
          >
...
export function MemberCouponList() {
  ...
  return (
    <div>
        ...
        {FILTERS.map((item) => {
          ...
          return (
            <button
              ...
              className={`rounded-full text-xs font-body px-3.5 py-1.5 border transition-colors ${
                filter === item.key
                  ? "border-sf-accent bg-sf-selected text-sf-ink"
                  : "border-sf-line-strong text-sf-text hover:bg-sf-selected"
              }`}
            >
      ...
      {isLoading ? (
        <p className="text-sm text-sf-muted font-body text-center py-12">載入優惠券中...</p>
      ) : error ? (
        <p className="text-sm text-red-600 font-body text-center py-12">載入優惠券失敗</p>
      ) : visible.length === 0 ? (
        <p className="text-sm text-sf-muted font-body text-center py-12">
        ...
        <div className="space-y-3">
          {visible.map((coupon) => {
            ...
            return (
              <div
                key={coupon.id}
                className={`overflow-hidden rounded-lg bg-white border border-sf-line flex items-stretch ${muted ? "opacity-70" : ""}`}
              >
                <div className="w-24 shrink-0 flex flex-col items-center justify-center border-r border-dashed border-sf-line-strong bg-sf-cream px-2 py-4">
                  <span className="text-[0.65rem] font-body text-sf-muted">折</span>
                  <span className="text-xl text-sf-ink" style={{ fontFamily: "'Noto Serif TC', serif" }}>
                    ${coupon.discountAmount.toLocaleString()}
                  </span>
                </div>
                <div className="flex-1 min-w-0 px-4 py-3">
                  <div className="flex items-start justify-between gap-2">
                    <p className="text-sm font-medium text-sf-ink font-body">{coupon.name}</p>
                    <span
                  ...
                  <p className="text-xs text-sf-muted font-body mt-1">{formatCouponMinimum(coupon.minOrderAmount)}</p>
                  <p className="text-xs text-sf-muted font-body mt-0.5">
      ...
      <p className="mt-4 text-xs text-sf-muted font-body">結帳時可選擇一張優惠券折抵商品金額（不含運費）。</p>
    </div>
  );
}
