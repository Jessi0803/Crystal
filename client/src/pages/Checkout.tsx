...
export default function Checkout() {
  ...
  const hasCustomDepositItem =
    items.length > 0 &&
    items.some(item => CUSTOM_PRODUCT_IDS.includes(item.product.id));
  ...
  const { data: liveProducts = [] } = trpc.product.list.useQuery(undefined, {
  ...
  const getTwoItemFreeShippingEligibility = (
    productId: string,
    fallback?: boolean
  ...
  const feeSummary = calcCheckoutFees({
    items: items.map(i => ({
      ...
      price: i.unitPrice,
      quantity: i.quantity,
      twoItemFreeShippingEligible: getTwoItemFreeShippingEligibility(
        i.product.id,
        i.product.twoItemFreeShippingEligible
      ),
    })),
    checkoutRegion,
    shippingMethod,
  ...
  const validate = () => {
    ...
    if (
      ...
      paymentMethod === "atm" &&
      !/^\d{5}$/.test(form.transferLastFive.trim())
    ) {
    ...
    if (isCustomDepositCheckout) {
      if (
        ...
        !/^09\d{8}$/.test(form.buyerPhone.replace(/\s/g, ""))
      )
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await createAndPay.mutateAsync({
        ...
        paymentMethod,
        shippingMethod: isCustomDepositCheckout
          ? "home"
        ...
        receiverZipCode:
          ...
          shippingMethod === "home"
            ...
            : undefined,
        intlCountry:
          !isCustomDepositCheckout && checkoutRegion === "overseas"
            ? form.intlCountry
            : undefined,
        intlAddrLine1:
          !isCustomDepositCheckout && checkoutRegion === "overseas"
        ...
        intlPostalCode:
          !isCustomDepositCheckout && checkoutRegion === "overseas"
            ...
            : undefined,
        transferLastFive:
          checkoutRegion === "domestic" && paymentMethod === "atm"
            ? form.transferLastFive.trim()
            : undefined,
        transferReceiptImageBase64:
          checkoutRegion === "domestic" && paymentMethod === "atm"
            ? transferReceipt?.dataBase64
            : undefined,
        transferReceiptImageContentType:
          checkoutRegion === "domestic" && paymentMethod === "atm"
            ? transferReceipt?.contentType
            : undefined,
        ...
        items: items.map(i => ({
          ...
          claspType: i.claspType,
          fitPreference: i.fitPreference,
                ...
                {paymentMethod === "atm" && (
                  <div className="mt-4 border border-blue-200 bg-blue-50 p-4">
                    <div className="mb-5 border border-blue-200 bg-white p-4">
                      <p className="text-xs tracking-[0.16em] font-body text-blue-800 mb-3">
                        轉帳資訊
                      </p>
                    ...
                    <input
                      ...
                      onChange={e =>
                        setForm(f => ({
                          ...f,
                          transferLastFive: e.target.value
                            .replace(/\D/g, "")
                            .slice(0, 5),
                        }))
                      }
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {(item.purchaseOptionLabel ||
                        ...
                        <p className="text-[0.65rem] font-body text-[oklch(0.45_0_0)] mt-0.5">
                          ...
                          {item.wristSizeSelections
                            ?.map(
                              selection =>
                                `${selection.label} ${selection.value} cm`
                            )
                            .join(" · ") ?? ""}
                          {(item.wristSize ||
                            item.wristSizeSelections?.length) &&
                          item.claspType
                            ? " · "
                            : ""}
                          {item.claspType === "elastic"
                            ? "彈力繩"
                            : item.claspType === "lobster"
                              ? "龍蝦扣"
                              : item.claspType === "magnetic"
                                ? "磁扣"
                                : ""}
                          {(item.wristSize ||
                            item.wristSizeSelections?.length ||
                          ...
                          item.fitPreference
                            ? " · "
                            : ""}
                          {item.fitPreference === "just-right"
                            ? "剛好"
                            : item.fitPreference === "loose"
                              ? "微鬆"
                              : ""}
                        </p>
                      )}
